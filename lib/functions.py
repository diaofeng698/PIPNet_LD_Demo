...
def train_model(det_head, net, train_loader, criterion_cls, criterion_reg, cls_loss_weight, reg_loss_weight, num_nb, optimizer, num_epochs, scheduler, save_dir, save_interval, device):
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            if det_head == 'pip':
                ...
                loss_map, loss_x, loss_y, loss_nb_x, loss_nb_y = compute_loss_pip(
                    outputs_map, outputs_x, outputs_y, outputs_nb_x, outputs_nb_y, labels_map, labels_x, labels_y, labels_nb_x, labels_nb_y, criterion_cls, criterion_reg, num_nb)
                loss = cls_loss_weight*loss_map + reg_loss_weight*loss_x + reg_loss_weight * \
                    loss_y + reg_loss_weight*loss_nb_x + reg_loss_weight*loss_nb_y
            else:
        ...
        print('[Epoch {:d}/{:d}] <Epoch loss: {:.6f}>'.format(epoch,
              num_epochs-1, epoch_loss))
        logging.info(
            '[Epoch {:d}/{:d}] <Epoch loss: {:.6f}>'.format(epoch, num_epochs-1, epoch_loss))
...
def forward_pip(net, inputs, preprocess, input_size, net_stride, num_nb):
    net.eval()
    with torch.no_grad():
        outputs_cls, outputs_x, outputs_y, outputs_nb_x, outputs_nb_y = net(
            inputs)
        ...
        tmp_x = (max_ids % tmp_width).view(-1, 1).float() + \
            outputs_x_select.view(-1, 1)
        tmp_y = (max_ids//tmp_width).view(-1, 1).float() + \
            outputs_y_select.view(-1, 1)
        tmp_x /= 1.0 * input_size / net_stride
        tmp_y /= 1.0 * input_size / net_stride

        tmp_nb_x = (max_ids % tmp_width).view(-1,
                                              1).float()+outputs_nb_x_select
